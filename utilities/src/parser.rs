...
// parser to look for instances of the word "shelf" that are not preceded by the phrase "elf on a"
fn shelf_no_elf(input: &str) -> IResult<&str, &str> {
    recognize(terminated(elf_on_a, shelf))(input)
}
...
// combinator to parse the input string for instances of the word "shelf"
...
mod tests {
    ...
    fn test_parse_shelves_with_no_elves() {
        let input = "there is an elf on a shelf on an elf. there is also another shelf in Belfast.";
        let shelves_no_elves = parse_shelves_with_no_elves(input).unwrap();
        let shelf_no_elf_count = shelves_no_elves.1.len();
        assert_eq!(shelf_no_elf_count, 1);
    }
}
